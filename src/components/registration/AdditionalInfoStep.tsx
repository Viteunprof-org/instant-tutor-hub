...
export default function AdditionalInfoStep({ userType, data, onDataChange, onNext, onBack, isValid }: AdditionalInfoStepProps) {
  const title = userType === "parent" ? "Votre enfant" : "Informations complémentaires";
  const subtitle = userType === "parent" ? "Entrez les informations concernant votre enfant !" : "Quelques informations supplémentaires";
    ...
    <Card className="w-full max-w-md">
      ...
      <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {gradeOptions.slice(0, 4).map((grade) => (
                  <Button
                    ...
                    className={`h-10 text-sm ${data.childGrade === grade.value ? "bg-vup-navy text-white" : "hover:bg-secondary"}`}
            ...
            <div className="grid grid-cols-4 gap-2">
              {gradeOptions.slice(0, 4).map((grade) => (
                <Button
                  ...
                  className={`h-10 text-sm ${data.grade === grade.value ? "bg-vup-navy text-white" : "hover:bg-secondary"}`}
        ...
        <div className="space-y-2">
          ...
          <Input id="phone" type="tel" placeholder="06 12 34 56 78" value={data.phone} onChange={(e) => onDataChange("phone", e.target.value)} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="sponsorCode">
            Code de parrainage <span className="text-muted-foreground">(facultatif)</span>
          </Label>
          <Input
            id="sponsorCode"
