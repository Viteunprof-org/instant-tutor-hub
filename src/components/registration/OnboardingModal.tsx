  ...
  },
  {
    ...
    content: [
      ...
      },
      {
        ...
        description:
          "Vous vous apprêtez à créer un compte pour votre enfant. N'oubliez pas de lui faire télécharger l'app et de lui donner les identifiants !",
      },
      {
...
export default function OnboardingModal({ isOpen, onClose, userType }: OnboardingModalProps) {
  ...
  const steps = userType === "student" ? studentOnboardingSteps : userType === "parent" ? parentOnboardingSteps : teacherOnboardingSteps;
        ...
        <Card className="w-full">
          <CardContent className="p-6">
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-vup-navy mb-8">{steps[currentStep].title}</h2>
            </div>
            ...
            <div className="space-y-6 mb-8">
              {steps[currentStep].content.map((item, index) => (
                <div key={index} className="flex items-start space-x-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-vup-navy mb-1">{item.title}</h3>
            ...
            {steps[currentStep].note && <p className="text-center text-sm text-muted-foreground italic mb-6">{steps[currentStep].note}</p>}

            <div className="space-y-3">
              <Button onClick={handleNext} className="w-full bg-vup-navy text-white hover:bg-vup-navy/90">
              ...
              <div className="flex justify-center space-x-2">
                {steps.map((_, index) => (
                  <div key={index} className={`w-2 h-2 rounded-full transition-colors ${index === currentStep ? "bg-vup-navy" : "bg-muted"}`} />
                ))}
              </div>
            </div>
          </CardContent>
